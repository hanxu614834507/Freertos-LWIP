...
u8 ETH_MACDMA_Config(void)
{
    ...
    ETH_StructInit(&ETH_InitStruct);            //初始化网络为默认值  
    ...
    ETH_InitStruct.ETH_RetryTransmission = ETH_RetryTransmission_Disable;        //关闭重传功能
    ETH_InitStruct.ETH_AutomaticPadCRCStrip = ETH_AutomaticPadCRCStrip_Disable;  //关闭自动去除PDA/CRC功能 
    ETH_InitStruct.ETH_ReceiveAll = ETH_ReceiveAll_Disable;                      //关闭接收所有的帧
	...
	rval=ETH_Init(&ETH_InitStruct,DP83848_PHY_ADDRESS);		//配置ETH
