...
#define SPI_FLASH_PORTSCK	 GPIOB
...
#define W25X_WriteDisable		0x04 			//写失能命令
...
#define W25X_JedecDeviceID		0x9F 			//读取器件ID
SFLASH_T g_tSF;
...
static void sf_CfgSPIForSFlash(void);
...
void bsp_spi_flash_init(void)
{
    ...
    GPIO_PinAFConfig(SPI_FLASH_PORTSCK, FLASH_SCK_PINSource,  GPIO_AF_SPI1);
    ...
    GPIO_InitStructure.GPIO_Pin = FLASH_SCK_PIN | FLASH_MISO_PIN | FLASH_MOSI_PIN;
		GPIO_Init(SPI_FLASH_PORTSCK, &GPIO_InitStructure);
...
void bsp_sf_PageWrite(uint8_t * _pBuf, uint32_t _uiWriteAddr, uint16_t _usSize)
{
    ...
    */
    void sf_ReadInfo(void)
    {
        /* 自动识别串行Flash型号 */
        {
            g_tSF.ChipID = bsp_sf_ReadID(); /* 芯片ID */
